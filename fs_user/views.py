...
def register(request):
    if request.method == 'GET':
        return render(request, 'fs_user/register.html', {'title': '鲜花-注册'})
    elif request.method == 'POST':
        username = request.POST.get('user_name')
        pwd = request.POST.get('pwd')
        cpwd = request.POST.get('cpwd')
        email = request.POST.get('email')
        address = request.POST.get('u_shou_address')
        postcode = request.POST.get('u_postcode')
        ...
        user.u_email = email
        user.u_shou_address = address
        user.u_postcode = postcode
        user.u_phone = phone
        user.save()
...
def login(request):
    if request.method == 'GET':
        return render(request, 'fs_user/login.html', {'title': '鲜花-登录'})
    elif request.method == 'POST':
