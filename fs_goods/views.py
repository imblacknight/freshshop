...
def index(request):
    ...
    ggood=GoodsInfo.objects.all()
    goods = []
    for category in categorys:
        # 根据分类查询对应的所有商品信息
        hot_goods = category.goodsinfo_set.all().order_by('-g_click')[:2]
        show_goods = category.goodsinfo_set.all().order_by('g_price')[:4]
        show_pic = category.goodsinfo_set.all()
        dic = {'hot_goods': hot_goods, 'show_goods': show_goods, 'category': category.title, 'category_id': category.id,'show_pic':show_pic}
        goods.append(dic)
...
#         return render(request, 'fs_goods/index.html', {'title': '鲜花-首页'})
#     elif request.method == 'POST':
#         search_content = request.POST.get('search1')
#         search1 = GoodsInfo.objects.filter(g_title=search_content)
#         print(search1)
